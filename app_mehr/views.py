...
from .models import LoanRequest, Profile, Message, Trans, Sandogh
...
@api_view(['GET'])
def sandogh(request):
    sandogh = Sandogh.objects.first()
    serializer = SandoghSerializer(sandogh,  many=False)
...
class LoanRequestUpdateView(UpdateView):
    model = LoanRequest
    fields = ['loan_type', 'request_type', 'loan_amount',
              'deposit_amount', 'status', 'due_date']
    template_name = 'loan_request_update.html'  # نام قالب HTML برای فرم آپدیت
    # پس از به‌روزرسانی به این URL هدایت شود
    success_url = reverse_lazy('loan_request_list')


class LoanRequestDeleteView(DeleteView):
    model = LoanRequest
    template_name = 'loan_request_confirm_delete.html'  # نام قالب HTML برای تایید حذف
    # پس از حذف به این URL هدایت شود
    success_url = reverse_lazy('loan_request_list')
